...
for year in years_sorted:
    ...
    # stats
    year_mean = subset.mean()
    year_median = subset.median()

    # scale
    y_max = max(hist_y.max(), y_density_scaled.max()) * 1.1
    year_to_ymax[year] = y_max

    # frame with histogram, density, and mean/median lines
    frame = go.Frame(
        name=str(year),
        data=[
            go.Bar(x=hist_x_center, y=hist_y, marker_color="#69b3ff", name="Histogram"),
            go.Scatter(
                x=x_range, y=y_density_scaled, mode="lines",
                line=dict(color="darkblue", width=3), name="Density"
            ),
            go.Scatter(
                x=[year_median, year_median], y=[0, y_max],
                mode="lines", line=dict(color="red", dash="dot", width=2),
                name="Median"
            ),
            go.Scatter(
                x=[year_mean, year_mean], y=[0, y_max],
...
# === 3) First frame traces ===
first_year = int(frames[0].name)
subset0 = df[df["year"] == first_year]["median"].dropna()
h_y, h_x = np.histogram(subset0, bins=50)
h_xc = 0.5 * (h_x[:-1] + h_x[1:])
kde0 = gaussian_kde(subset0)
x0 = np.linspace(subset0.min(), subset0.max(), 200)
y0 = kde0(x0)
y0_scaled = y0 * h_y.max() / y0.max()
mean0 = subset0.mean()
median0 = subset0.median()

fig = go.Figure(data=[
    go.Bar(x=h_xc, y=h_y, marker_color="#69b3ff", name="Histogram"),
    go.Scatter(x=x0, y=y0_scaled, mode="lines", line=dict(color="darkblue", width=3), name="Density"),
    go.Scatter(x=[median0, median0], y=[0, year_to_ymax[first_year]],
               mode="lines", line=dict(color="red", dash="dot", width=2), name="Median"),
    go.Scatter(x=[mean0, mean0], y=[0, year_to_ymax[first_year]],
               mode="lines", line=dict(color="green", dash="dash", width=2), name="Mean"),
])
...
updatemenus = [{
    ...
    "direction": "left", "x": 0.05, "y": -0.12, "showactive": False,
}]
...
fig.update_layout(
    ...
    yaxis=dict(range=[0, year_to_ymax[first_year]], showgrid=False),
...
# === 6) Display + Save ===
fig.show()
fig.write_html("Graph1_sydney_price_distribution_with_mean_median_lines.html")

print("Saved: Graph1_sydney_price_distribution_with_mean_median_lines.html")
